...
export default function OutpatientclinicIndex() {
    const [loading, setLoading] = useState(true);
    const mysal = withReactContent(swal);
    const [outpatientclinicData, setOutpatientclinicData] = useState([]);
    const fetchOutpatientclinicData = async () => {
        setLoading(true);
        try {
            const response = await Outpatientclinic.getAll();
    ...
    function handleDelete(outpatientclinicid, name) {
        mysal.fire({
            ...
            preConfirm: async () => {
                try {
                    var res = await Outpatientclinic.destroy(outpatientclinicid);
    ...
    return (
        <>
            <div class="card shadow-sm">
                <div class="card-body">
                    <h4 class="card-title">Quản lý phòng khám</h4>
                                ...
                                {!loading &&
                                    (
                                        outpatientclinicData.map(item => {
                                            return (
                                                <tr key={item.outpatientclinicid}>
                                                    <th className="align-middle" scope="row">{item.outpatientclinicid}</th>
