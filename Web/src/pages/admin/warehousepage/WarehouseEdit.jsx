...
export default function WarehouseEdit() {
    ...
    async function getWarehouse() {
        try{
            const res = await Warehouse.getDetail(id);
            const did = [];
            res.data?.usermanagers?.map(item => {
                did.push(item.userid);
            });
            setUserIds(did);
    ...
    async function getListUser() {
        try {
            ...
            const options = res.data.map((item) => ({
                value: item.userid,
                label: `${item.userid}-${item.profile?.fullname}`,
            }));
            setUserData(options);
        } catch (error) {
    ...
    const updateWarehouse = async (e) => {
        ...
        try {
            ...
            const response = await Warehouse.update(data,id);
    ...
    return (
        <>
            <div className="d-flex mb-1">
                ...
                <h3 className="m-0">Chỉnh sửa kho dược</h3>
                <div className="d-flex flex ms-auto">
                    <button type="button" disabled={loadingpost} className="btn btn-primary fs-5" onClick={updateWarehouse}> {loadingpost ? (<i className="fa-solid fa-spinner fa-spin"></i>) : ("Cập nhật")}</button>
                </div>
            </div>
            <form className="pb-2">
                <div className="row">
                    <div className="col-12">
                        ...
                        <div class="py-1">
                            ...
                            <select class="form-select border-primary" onChange={handleChangeInput} value={warehouseData.isactive} name="isactive" aria-label="Default select example">
