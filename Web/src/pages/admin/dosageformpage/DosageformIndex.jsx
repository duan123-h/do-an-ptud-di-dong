...
export default function DosageformIndex() {
    ...
    const fetchDosageformData = async () => {
        setLoading(true);
        try {
            const response = await Dosageform.getAll();
            console.log(response.data);
            setDosageformData(response.data);
        } catch (error) {
    ...
    function handleDelete(Dosageformid, name) {
        mysal.fire({
            title: `Bạn có chắc muốn xóa dạng bào chế thuốc ${name} không?`,
            ...
            preConfirm: async () => {
                try {
                    await Dosageform.destroy(Dosageformid);
                    mysal.fire('Đã xóa!', 'dạng bào chế thuốc đã được xóa.', 'success');
                    fetchDosageformData();
                } catch (error) {
                    swal.fire('Lỗi!', 'Xóa dạng bào chế thuốc không thành công.', 'errors');
                }
            },
        })
    }
    return (
        <>
            <div class="card shadow-sm">
                <div class="card-body">
                    <h4 class="card-title">Quản lý dạng bào chế thuốc</h4>
                    ...
                    <div className="table-responsive">
                        <table className="table" style={{ Width: '100%' }} >
                            <thead className="border-bottom-2 border-dark">
                                <tr>
                                    <th scope="col">Mã dạng bào chế</th>
                                    <th scope="col">Tên dạng bào chế</th>
