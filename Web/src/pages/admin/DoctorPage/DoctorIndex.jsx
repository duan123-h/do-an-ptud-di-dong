...
export default function DoctorIndex() {
    ...
    function handleDelete(Doctorid, name) {
        mysal.fire({
            ...
            preConfirm: async () => {
                try {
                    await StaffService.destroy(Doctorid);
                ...
                <div class="card-body">
                    ...
                    <div className="d-sm-flex flex-row d-block align-items-center">
                        <form action="" method="get" className="d-flex flex-row align-items-center">
                            ...
                            <Link to="/his/Doctor" className="btn btn-primary ms-2"><i className="fa-solid fa-rotate-right"></i></Link>
                        </form>
                        <Link to="/his/Doctor/create" className="btn btn-success mx-0 my-2 ms-sm-auto text-nowrap"><i className="fa-solid fa-plus me-2"></i>Thêm</Link>
                    </div>
                                ...
                                {!loading &&
                                    (
                                        doctorData.map(item => {
                                            return (
                                                <tr key={item.staffid}>
                                                    <th className="align-middle" scope="row">{item.staffid}</th>
                                                    ...
                                                    <td className="align-middle" style={{ minWidth: '110px', Width: '110px' }}>
                                                        <Link to={`/his/Doctor/edit/${item.staffid}`} className="btn btn-primary mx-1 px-2 py-1"><i className="fa-regular fa-edit fs-6"></i></Link>
                                                        <button className="btn btn-danger mx-1 px-2 py-1"><i
                                                            className="fa-regular fa-trash-can fs-6" onClick={() => handleDelete(item.staffid, item.fullname)}></i></button>
                                                    </td>
                                                </tr>
                                            );
                                        })
